...
import { remoteChatState } from '../../remoteBridge/common/remoteChatState';
...
Learn more about [GitHub Copilot](https://docs.github.com/copilot/using-github-copilot/getting-started-with-github-copilot?tool=vscode&utm_source=editor&utm_medium=chat-panel&utm_campaign=2024q3-em-MSFT-getstarted) in [Visual Studio Code](https://code.visualstudio.com/docs/copilot/overview). Or explore the [Copilot walkthrough](command:github.copilot.open.walkthrough).`,
	...
	private getChatParticipantHandler(id: string, name: string, defaultIntentIdOrGetter: IntentOrGetter): vscode.ChatExtendedRequestHandler {
		return async (request, context, stream, token): Promise<vscode.ChatResult> => {
			...
			const remoteModelConfiguration = remoteChatState.consumeNextModelConfiguration(sessionResource);
			if (remoteModelConfiguration) {
				const permissionLevel = remoteModelConfiguration.permissionLevel;
				const enabledTools = remoteModelConfiguration.enabledTools;
			...
			try {
				...
				const observedStream = ChatResponseStreamImpl.spy(
					stream,
					part => remoteChatState.part(sessionResource, request.id, part),
					undefined,
					usage => remoteChatState.usage(sessionResource, request.id, usage, request.model.maxInputTokens),
				);
				const handler = this.instantiationService.createInstance(ChatParticipantRequestHandler, context.history, request, observedStream, token, { agentName: name, agentId: id, intentId }, () => context.yieldRequested, telemetryMessageId);
				...
				if ((result as ICopilotChatResultIn).metadata?.shouldAutoSwitchToAuto) {
					...
					if (switchedRequest.model?.id !== previousModelId) {
						...
						const retryHandler = this.instantiationService.createInstance(ChatParticipantRequestHandler, context.history, request, observedStream, token, { agentName: name, agentId: id, intentId }, () => context.yieldRequested, telemetryMessageId);
