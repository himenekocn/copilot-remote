...
async function main() {
	...
	if (!fs.existsSync(baseCachePath)) {
		console.warn(`Simulation cache is not present at ${baseCachePath}; extension packaging can continue, but simulation tests that need the cache will be unavailable.`);
	}
