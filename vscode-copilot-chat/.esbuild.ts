...
async function copyRuntimeAssets(): Promise<void> {
	...
	const assets = [
		{
			source: './node_modules/@github/blackbird-external-ingest-utils/pkg/nodejs/external_ingest_utils_bg.wasm',
			destination: './dist/external_ingest_utils_bg.wasm',
		},
	];

	for (const asset of assets) {
		try {
			await copyFile(path.join(import.meta.dirname, asset.source), path.join(import.meta.dirname, asset.destination));
		} catch (error) {
			console.error(`Error copying runtime asset ${asset.source}:`, error);
			throw error;
		}
	}
}
